...
export const FIND_CHAT_MEMBERS_BY_CHAT_ID = `SELECT * FROM chats WHERE id = ?;` // will retrieve all the chat members of a specific chat.

export const DELETE_CHAT_MEMBER = `DELETE FROM chats WHERE chat_id = ? AND user_id = ?;` // will remove a member with id: user_id from a chat.
