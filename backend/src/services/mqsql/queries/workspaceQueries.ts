...
export const FIND_WORKSPACE = `SELECT * FROM workspaces WHERE creator_id = ? && name = ? LIMIT 1;` // finds particular workspace of a user
...
//TODO:
export const FIND_USER_WORKSPACES = `SELECT w.* FROM workspaces w inner join members m WHERE m.user_id = ? && w.id = m.workspace_id ;`
